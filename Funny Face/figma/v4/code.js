...
const C = {
  ...
  // v4 — màn chơi đảo sang NỀN SÁNG. Đo lại theo đúng vạch v3 thật sự giữ (xem
  // Funny Face/docs không có; bản đối chiếu nằm ở app: docs/plan/11-v4-light-play-screens.md):
  //   onLight trên bg 15.47:1 · trên bgTop 12.51:1   (v3 onDark: 10.32 / 13.17)
  //   onLightSub trên bg 6.15:1                      (v3 onDarkSub: 6.61)
  // brandSoft KHÔNG dùng làm chip đang chọn được nữa: 1.2:1 so với nền, chip chìm.
  bg:        'f1ecff',  // đầu sáng của gradient màn chơi
  bgTop:     'dcd2ff',  // đầu đậm
  surface:   'ffffff',  // thẻ
  surfaceHi: 'ffffff',  // chip chưa chọn — chữ gánh tương phản, không phải nền
  line:      'b9a9ee',
  chipSel:   '9a6cff',  // = brand, chip đang chọn
  onChipSel: '220d4b',  // 4.86:1 trên chipSel; nhãn 12sp bold nên cần 4.5
  // các token nền tối cũ, giữ cho màn camera/video
  bgDark:    '2b2252',
  surfaceDark:'3a2f6c',
  surfaceHiDark:'4b3d8c',
  lineDark:  '5f4fa8',
  ...
  paperLine: 'ede6dc',
  // ── Chữ ───────────────────────────────────────────────────────────────────
  // v4: màn chơi là nền sáng nên chữ mặc định đảo sang tối. Giữ onMedia cho chữ
  // nằm trên camera/video — thứ đó vẫn phải trắng dù theme đổi thế nào.
  onMedia:   'f5f3ff',
  onDark:    '1a1523',
...
// v4: tô lại icon thành một màu cụ thể. Dùng cho nút play/pause màn Sound — SVG của nó vẽ
// bằng #f5f3ff (token OnDark), trên nền lavender sáng chỉ còn 1.1:1 nên gần như vô hình.
// tintIcon() không cứu được vì nó chỉ lật màu TỐI thành sáng, không lật chiều ngược lại.
// v4 · nút back trên nền sáng — thắng A/B (B2: đĩa trắng + bóng mềm, 98/120).
// Nền lavender làm mũi tên trần biến mất; đĩa trắng tách nền và báo rõ đây là chỗ bấm,
// mà vẫn nhẹ hơn tiêu đề — đúng thứ bậc của một nút điều hướng phụ.
// Mũi tên bằng 52% đường kính theo góp ý craft (bản đầu chỉ 34%, nhìn như đĩa rỗng),
// lệch phải 1px để đầu nhọn không làm chữ trông lệch trái.
// ring: màu viền, dùng cho bề mặt mà đĩa trắng không tự tách được bằng độ sáng.
// Trên lavender #DCD2FF đĩa đạt 1.43:1 và bóng đổ là đủ; trên giấy kem #FDF9F3 chỉ còn
// 1.05:1 — không còn ranh giới nào, nên ở đó đĩa bắt buộc phải có viền.
function backChip(parent,x,y,dia,ring){
  dia=dia||40;
  var e=figma.createEllipse();e.name='btnBack chip';e.x=x;e.y=y;e.resize(dia,dia);
  e.fills=[solid('ffffff')];
  if(ring){e.strokes=[solid(ring)];e.strokeWeight=1.5;}
  e.effects=[{type:'DROP_SHADOW',color:{r:0.36,g:0.13,b:0.71,a:0.22},
              offset:{x:0,y:2},radius:6,spread:0,visible:true,blendMode:'NORMAL'}];
  parent.appendChild(e);
  var g=Math.round(dia*0.52);
  var ic=iconColored('ic_back_1',x+(dia-g)/2+1,y+(dia-g)/2,g,g,'#5b21b6');
  if(ic)parent.appendChild(ic);
  return e;
}
function iconColored(name,x,y,w,h,hex){if(typeof ICONS==='undefined'||!ICONS[name])return null;
  var sv=ICONS[name].replace(/(fill|stroke)="#(?:[0-9a-fA-F]{6})"/g,function(m,att){return att+'="'+hex+'"';})
                    .replace(/(fill|stroke)="#[0-9a-fA-F]{2}(?:[0-9a-fA-F]{6})"/g,function(m,att){return att+'="'+hex+'"';});
  var n=figma.createNodeFromSvg(sv);n.name=name;n.x=x;n.y=y;n.resize(w,h);return n;}
...
function screenBg(f,h){const bg=rect('bg',0,0,SCREEN_W,h,{gradient:[C.bgTop,C.bg],gradientDir:'v'});f.appendChild(bg);const pat=rect('hoạ tiết mặt',0,0,SCREEN_W,h,{image:'pattern_overlay',scaleMode:'TILE'});f.appendChild(pat);return{bg:bg,pat:pat};}
