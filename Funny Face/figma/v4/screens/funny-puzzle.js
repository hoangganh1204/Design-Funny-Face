...
screen(
  ...
  (f) => {
    ...
    backChip(back, 4, 4, 40);   // v4: nền sáng -> đĩa trắng, mép trái về keyline 16dp
    ...
    FNP_LIST_DATA_V1.forEach((item, i) => {
      ...
      add(card, plate);
      // Chữ này nằm trên ARTWORK TỐI của nhân vật, không phải trên nền màn. v4 đảo nền
      // sáng nên textHi thành tối và tên chìm hẳn vào ảnh. audit-contrast không bắt được
      // vì nó dò nền đục gần nhất và không lấy mẫu được pixel trong image fill.
      add(card, text(item.name, 1, CARD - 1 - 37, {
        size: 14, weight: 700, font: 'main', color: C.onMedia,
