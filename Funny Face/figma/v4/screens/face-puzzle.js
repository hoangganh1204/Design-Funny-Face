...
(function () {
  ...
  var LV_W = 328, LV_H = (328 * 360) / 1008;              // bg_level_* thật = 1008x360px
  ...
  function recordingButton(f, x, y, opt) {
    ...
    if (opt.arcSweepDeg) {
      ...
      vpath(g, 0, 0, 80, 80, '80 80', d, { stroke: '02bbfa', sw: 12, name: 'progress-arc' });
    }
    ...
    function (f) {
      ...
      btnLabels.forEach(function (label) {
        add(d, text(label, 16, btnY, {
          size: 14, weight: 600, font: 'main', color: '0b57d0', w: CARD_W - 32, h: btnH,
  ...
  screen('28 · Level Picker · Mặc định (level_picker_skipped=false → CÓ hiển thị)',
    ...
    function (f) {
      screenBg(f, SCREEN_H);
      backChip(f, 16, 16, 40);   // v4: nền sáng
      add(f, text('Select level', 0, 12, {
        size: 20, weight: 700, font: 'main', color: C.textHi, w: SCREEN_W, h: 48,
        align: 'CENTER', valign: 'CENTER',
      }));
      var y = 240;
      add(f, img('bg_level_easy', 16, y, LV_W, LV_H, 'FIT')); y += LV_H + 28;
      add(f, img('bg_level_intermediate', 16, y, LV_W, LV_H, 'FIT')); y += LV_H + 28;
      add(f, img('bg_level_difficulty', 16, y, LV_W, LV_H, 'FIT')); y += LV_H + 28;
  ...
  function nativeFakeFullInter(f, placementLabel) {
    ...
    add(g, rect('cta', ctaX, ctaY, ctaW, 45, { fill: '4285f4', radius: 16 }));
  ...
  screen('31b · Level Picker · Chờ ad fullscreen sau khi bấm 1 level',
    ...
    function (f) {
      ...
      add(f, rect('disabled-overlay', 20, y, LV_W, LV_H, { fill: C.bg, opacity: 0.6 }));
      y += LV_H + 16;
      add(f, img('bg_level_intermediate', 16, y, LV_W, LV_H, 'FIT')); y += LV_H + 28;
      add(f, img('bg_level_difficulty', 16, y, LV_W, LV_H, 'FIT')); y += LV_H + 28;
      adSlot(f, 20, y, LV_W, 100, 'native_level');
