...
screen('60 · Result · Skeleton ad (t=0)',
  ...
  (f) => {
    ...
    add(f, rect('scrim dưới', 0, 620, SCREEN_W, 180, { gradient: ['0014101f', 'e614101f'], gradientDir: 'v' }));
    ...
    const btnSave = frame('btnSave', 16, saveY, SCREEN_W - 32, 58, { fill: C.action, radius: 29, shadow: 0.45 });
    add(f, btnSave);
    ...
    add(btnSave, text('Save', 158, 0, { size: 16, weight: 700, font: 'main', color: C.onAction, w: 140, h: 58, align: 'LEFT', valign: 'CENTER' }));
    ...
    const skeleton = frame('collapseHolderNative (skeleton, bg_ad_native_media_white)', 0, adY, SCREEN_W, 256, { fill: C.surface });
    add(f, skeleton);
...
dialog('D62 · PlayVideo · Popup xoá video (Delete video?)',
  ...
  (f) => {
    ...
    const card = dcard(f, 306, 164, { fill: C.surface, radius: R.dialog, name: 'popup_exit_confirm · Delete video', scrim: 0.6 });
    ...
    add(card, text('Cancel', 16, btnY, { size: 14, weight: 700, font: 'main', color: C.onMedia, w: btnW, h: btnH, align: 'CENTER', valign: 'CENTER' }));
...
screen('62 · VideoGallery · Rỗng (empty)',
  ...
  (f) => {
    ...
    dashRect(f, (SCREEN_W - 240) / 2, 196, 240, 228, 28, 'a892e0', 2);
