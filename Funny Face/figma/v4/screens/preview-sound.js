...
// v4 · nút back trên nền sáng (Sound Picker). Trước đây là mũi tên 24x24 trần tại
// (24,24); nền lavender của v4 nuốt mất nó. Nay là đĩa trắng 40dp ở keyline 16dp —
// bản thắng A/B (B2, 98/120).
...
function nativeAdCard(adFrame) {
  ...
  add(adFrame, rect('ad badge (bg_text_view_ads)', badgeX, badgeY, badgeW, badgeH, { fill: C.white, radius: 3 }));
...
function soundItem(f, y, entry, selected) {
  ...
  // v4: nền danh sách đã là lavender sáng, icon gốc màu #f5f3ff chìm hẳn (1.1:1).
  // #5b21b6 = BrandDeep, 7.78:1, khớp luôn màu tím của checkbox bên trái.
  add(f, iconColored(playIc, SCREEN_W - 32 - 16, y + (itemH - 24) / 2, 24, 24, '#5b21b6'));
}
...
dialog('D51 · Preview With Music · Xác nhận thoát (Go back?)',
  ...
  (f) => {
    ...
    const card = dcard(f, w, h, { y, radius: R.dialog, fill: C.surface, name: 'popup_exit_confirm' });
