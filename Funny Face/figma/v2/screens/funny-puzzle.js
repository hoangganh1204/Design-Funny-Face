...
function fnp_drawChrome(f, o) {
  ...
  if (o.countdownText) {
    add(f, text(o.countdownText, 150, 637, {
      size: 36, weight: 900, font: 'main', color: '5416cd',
  ...
  add(rec, ellipse('progress track (interpretive)', 6, 6, 68, 68, { stroke: '5416cd', strokeW: 12, strokeOpacity: 0.25 }));
  if (o.progressArcD) {
    vpath(rec, 0, 0, 80, 80, '80 80', o.progressArcD, { stroke: '5416cd', sw: 12, name: 'progress arc' });
  }
  ...
  (f) => {
    ...
    BTN.forEach((label, i) => {
      add(card, text(label, 24, btnTop + i * btnH, {
        size: 14, weight: 500, font: 'main', color: '7f1aab', w: DW - 48, h: btnH, align: 'CENTER', valign: 'CENTER',
      }));
    });
...
screen(
  ...
  (f) => {
    ...
    add(card, rect('cta (SDK chrome)', 16, SCREEN_H - 76, SCREEN_W - 32, 48, { fill: 'c126ea', radius: 24 }));
